...
def GetArray(key):
    ...
    try:
        array = data.split('<key>'+key+'</key>')[1].split('<dict>')[1].split(
            '</dict')[0].replace('    ', '')  # getting array from spitename
...
def GetSizeFromArray(array):
    ...
    size = array.split('<key>spriteSize</key>')[1].split('<string>')[1].split('</string')[
        0].replace('{', '').replace('}', '')  # getting spriteSize from sprite array
    x, y = int(size.split(',')[0]), int(
        size.split(',')[1])  # splitting data and getting x&y
    return x, y  # returning spriteSize


def GetPosFromArray(array):
    'Get sprite pos from plist array'
    size = array.split('<key>textureRect</key>')[1].split('<string>')[1].split('</string')[
        0].replace('{', '').replace('}', '')  # getting sprite position from sprite array
    x, y = int(size.split(',')[0]), int(
        size.split(',')[1])  # splitting data and getting x&y
    return x, y  # returning spriteSize
...
def IsSpriteRotated(array):
    ...
    # getting bool from file
    string = array.split('<key>textureRotated</key>')[1]
...
def GetGamesheetSize():
    ...
    strsize = GetArray('metadata').split(
...
def SaveSprite(key):
    ...
    if (rotated):
        size = size[::-1]
    # creating new image with size of target sprite
...
while 1:
    ...
    if (ui == '1'):  # load textures
        ...
        if not('-hd' in f) and not('-uhd' in f):
            # warn user if non-hd file used
            print('NOTE: You have chosen a low quality file')
        ...
        for i in data.split('<key>')[1:]:
            if not(i.split('</key>')[0] in ['aliases', 'spriteOffset', 'spriteSize', 'spriteSourceSize', 'textureRect', 'textureRotated', 'frames']):
                ...
                # append key to array if not blacklisted
                keys.append(i.split('</key>')[0])
        print('Done!')
